...
if __name__ == '__main__':
    ...
    def model(u_in, y_meas):
        ...
        pyro.module("G", G) # make this module trainable for pyro
        scale_e = torch.tensor(std_noise_V)#10*torch.ones_like(u_in, dtype=u_in.dtype)  # prior variance of e
        y_true = G(u_in)  # true y, which is the mean of the y_obs distribution

        y_obs = pyro.sample("y_obs", dist.Normal(y_true, scale_e).to_event(2), obs=y_meas)
        return y_obs
    ...
    def guide(u_in, y_meas):
        ...
        scale_e = torch.tensor(std_noise_V)
        y_true = G(u_in)  # true y, which is the mean of the y_obs distribution
    ...
    for itr in range(num_iter):
        #losses.append(svi.step(u_torch, y_meas_torch))
        loss = svi.step(u_torch, y_meas_torch)
        LOSS_ITR.append(loss)
        #var_e_curr = pyro.param("var_e").item()
        #VAR_ITR.append(var_e_curr)
        if itr % test_freq == 0:
            print(f'Iter {itr} | Current loss {loss:.4f}')
            pass
