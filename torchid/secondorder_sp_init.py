...
class SecondOrderDynamical(torch.autograd.Function):
    ...
    def forward(ctx, b_coeff, f_coeff, u_in, y_init, u_init):
        ...
        zi = lfiltic_vec(b_np, f_np, y_init.numpy(), u_init.numpy())  # initial condition
    ...
    def backward(ctx, grad_output):
        ...
        f_np = np.concatenate(([1.0], f_coeff.numpy()))
        d1_np = np.array([0.0, 1.0], dtype=np.float)
        d2_np = np.array([0.0, 0.0, 1.0], dtype=np.float)

        # compute forward sensitivities w.r.t. the b_i parameters
        sens_b1_np = sp.signal.lfilter(d1_np, f_np, u_in, axis=0)
        sens_b2_np = sp.signal.lfilter(d2_np, f_np, u_in, axis=0)
        sens_b = torch.cat((torch.as_tensor(sens_b1_np, dtype=u_in.dtype), torch.as_tensor(sens_b2_np, dtype=u_in.dtype)), dim=1)
        ...
        sens_f1_np = sp.signal.lfilter(d1_np, f_np, -y_out, axis=0)
        sens_f2_np = sp.signal.lfilter(d2_np, f_np, -y_out, axis=0)
        sens_f = torch.cat((torch.tensor(sens_f1_np, dtype=u_in.dtype), torch.tensor(sens_f2_np, dtype=u_in.dtype)), dim=1)
