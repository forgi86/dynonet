...
class LinearDynamicalSystem(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        sens_b = np.empty_like(u_in, shape=(N, n_b))
        sens_b[:, 0] = sp.signal.lfilter(d1_np, f_np, u_in, axis=0)[:, 0]
        sens_b[:, 1] = 0.0
        sens_b[1:, 1] = sens_b[:-1, 0]
        ...
        sens_f = np.empty_like(u_in, shape=(N, n_f))
        sens_f[:, 0] = sp.signal.lfilter(d1_np, f_np, -y_out, axis=0)[:, 0]
        sens_f[:, 1] = 0.0
        sens_f[1:, 1] = sens_f[:-1, 0]
